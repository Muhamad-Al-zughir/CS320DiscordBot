...
async def clear(interaction: discord.Interaction):                                          
    server = interaction.guild                                                                      # Establish Server and Voice Client Connection
    voice_channel = server.voice_client

    if voice_channel is None:                                                                       # Check if Bot is connected to voice channel, if not, there is no queue
        await interaction.response.send_message(f'There is no music to clear!')
    else:
        await voice_channel.disconnect()                                                            # Else, clear the bot and its song List
        songList.clear()
...
async def pause_yt(interaction: discord.Interaction):
    ...
    vcstatus = interaction.guild.voice_client                                                       # Establish Voice Client connection
    if vcstatus is None:                                                                            # Check Cases for none, if paused or playing song
        ...
        await interaction.response.send_message(f'Unknown error in PAUSE occured')                  # This should never happen, good debug message
